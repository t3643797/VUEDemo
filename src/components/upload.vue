...
export default {
  ...
  data() {
    return {
      options: {
        target: 'http://localhost:9291/api/Upload',
  ...
  methods: {
    ...
    //debugger
    if (!datas.Result) {
        this.$message({ message: datas.Message, type: 'error' });
        return
    }

    // 如果服务端返回需要合并
    if (datas.Result&&datas.Result=="needMerge") {
...
#global-uploader {
  ...
  /deep/.uploader-file-icon {
    ...
    &[icon='rar'] {
      background: url(../assets/rar-icon.png);
    }
    &[icon='unknown'] {
      background: url(../assets/unknown-icon.png);
    }
  }
